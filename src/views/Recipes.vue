<template>
  <div class="recipes-container">
    <canvas id="recipesBgCanvas" class="page-bg-canvas" />
    <h1 class="page-title">
    ...
    <div class="settings-panel">
      <div class="setting-item">
        <label>{{ i18n.t('recipes.settings.iconSize') }}</label>
        <input
      ...
      <div class="setting-item">
        <label>{{ i18n.t('recipes.settings.categoryDisplay') }}</label>
        <select
          v-model="categoryDisplayMode"
          class="category-select"
        >
          <option value="id">
            {{ i18n.t('recipes.settings.categoryId') }}
          </option>
          <option value="name">
            {{ i18n.t('recipes.settings.categoryName') }}
          </option>
        </select>
      </div>
    </div>
    ...
    >
      ...
      >
        <div
          class="group-header"
          @click="toggleGroup(group.producerId)"
        >
        ...
        <div
          v-if="isExpanded(group.producerId)"
          class="group-content"
        >
          <div
...
const displayCategories = computed(() => {
  ...
  return categories.value.map(cat => {
    return {
      key: cat,
      label: categoryDisplayMode.value === 'name' && categoryMap[cat] ? categoryMap[cat] : cat
    }
  })
})
...
onMounted(() => {
  const canvas = document.getElementById('recipesBgCanvas')
  if (canvas) {
